...
export const METHODS = [
  ...
  { id: 'zscore',   name: '正規化平均(偏差)', desc: '審査員ごとの甘辛（厳しい/甘い採点）を標準化して補正。' },
...
export function scoreItems(votes, items, criteria, settings) {
  ...
  if (method === 'zscore') {
    // 審査員ごとの甘辛を標準化。
    // 各投票者について、その人が付けた全出し物の合計点で平均・標準偏差を出し、
    // z = (total - 投票者平均) / 投票者標準偏差 に変換してから出し物ごとに平均する。
    ...
    const zByItem = new Map(); // itemId -> [z...]
